...
export default {
  ...
  actions: {
    async searchPlaces({ commit }, payload) {
      ...
      const handleSearchResult = (data, status) => {
        if (status === kakao.maps.services.Status.OK) {
          ...
          showToast('검색결과를 찾을 수 없습니다.')
        } else {
          commit('updateState', {
            datas: [],
          })
          showToast('검색 중 오류가 발생하였습니다.')
        }
      }
      ...
      if (location) {
        ...
        return ps.keywordSearch(
          keyword,
          (data, status) => handleSearchResult(data, status),
          {
            location: center,
            radius: 2000,
            sort: 'distance',
      ...
      ps.keywordSearch(keyword, (result, status) => {
        ...
        ps.keywordSearch(
          keyword,
          (data, status2) => handleSearchResult(data, status2),
          {
            location: center,
            radius: 2000,
          },
